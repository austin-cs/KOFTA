...
\noindent 為了驗證本研究方法的可行性，我們提出以下四個 RQ:

\begin{itemize}
    \item \textbf{RQ1：KOFTA 是否能夠找到未紀錄於文件中的選項或選項參數？}
    \item \textbf{RQ2：KOFTA 是否能有與 Wei-fuzz 相當的測試效果？}
    \item \textbf{RQ3：KOFTA 是否能夠幫助 Wei-fuzz 達到更高的測試覆蓋率？}
    \item \textbf{RQ4：多參數 forkserver 是否真的能夠減少多參數模糊測試的額外開銷？}
\end{itemize}
...
與過去多參數模糊測試相關的研究相比，KOFTA 除了無須人力借助，另一個主要的差別在於分析的「對象」。其他研究使用目標程式「使用手冊（\texttt{man} 指令）」或「\texttt{--help} 選項」的內容作為分析選項、選項參數的依據；但 KOFTA 分析的是目標程式原始碼、執行階段的動態回饋，我們認為這種方式產生的選項、選項參數更能符合實際的情況。尤其是當使用手冊等文件記載不周、資訊過時，產生的選項也會是錯誤或缺漏的。因此我們的 RQ1 為：KOFTA 是否能夠找到未紀錄於文件中的選項或選項參數？

我們統計了 KOFTA 在 16 個小時內使用過的選項、選項參數，並與「使用手冊（\texttt{man} 指令）」或「\texttt{--help} 選項」的內容相比，統計數量並呈現於表格（Table \ref{tab:ud-opt-arg}）中。可以發現 KOFTA 透過靜態分析能夠找到所有記載於文件中的選項，並從 \texttt{objcopy}、\texttt{readelf}、\texttt{tiffcp} 三個目標程式
的原始碼中找到未記載於文件中的選項。污染源推論的效果更加顯著，可以從四個目標程式中找到共 37 個未記載於文件中的選項參數。
...
\subsection{Case Study: \texttt{readelf}}
...
\begin{lstlisting}[caption=\texttt{readelf} 解析未記載於文件中的選項, label=lst:readelf-udopt]
switch (c)
{
  case 'i':
...
\begin{table}[hb]
    ...
    \begin{tabularx}{0.8\textwidth} {
        ...
        \hline
    \end{tabularx}
    \caption{\texttt{tiffcrop} 未記載於文件中的選項參數}
    \label{tab:tiffcrop-udoptarg}
\end{table}

\subsection{Case Study: \texttt{tiffcrop}}
...
即使 KOFTA 能夠省下許多人工成本，甚至能夠發現未記載於文件中的選項、選項參數，幫助模糊測試探索更廣泛的輸入空間；但是這些好處也伴隨著由污染源推論產生的額外開銷。為了評估污染源推論所帶來的利弊，我們以 Wei-fuzz 為主要的比較對象，並提出 RQ2：KOFTA 是否能有與 Wei-fuzz 相當的測試效果？

我們統計並比較了 KOFTA 與 Wei-fuzz 測試總共 6 個目標程式在 16 個小時內的表現。為了避免隨機性影響結果，對於每個目標程式都會進行 4 輪的測試並計算平均路徑覆蓋率。從結果（Figure \ref{fig:kofta-vs-wei}）可以看出，6 個目標程式中正好有一半贏過 Wei-fuzz 的表現。進一步將結果整理成表格（Table \ref{tab:kofta-vs-wei}）後，雖然可以知道 KOFTA 整體平均路徑覆蓋率低於 Wei-fuzz 約 2\% 左右，但 KOFTA 還有能夠省下人工成本這個優點。同時能夠發現 KOFTA 對於特定的目標程式有明顯的喜好，測試結果最差的 \texttt{xmllint} 平均輸了 43\%；而最好的 \texttt{objcopy} 贏了高達 39\% 。
...
\begin{figure}[h!] % [h!]
    ...
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/16h/objdump-16h.png}
    \end{subfigure}
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/16h/objcopy-16h.png}
    \end{subfigure}
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/16h/readelf-16h.png}
    \end{subfigure}
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/16h/xmllint-16h.png}
    \end{subfigure}
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/16h/tiffcp-16h.png}
    \end{subfigure}
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/16h/tiffcrop-16h.png}
    \end{subfigure}
    \caption{KOFTA v.s. Wei-fuzz}
...
KOFTA 依據目標程式執行階段回饋的提示生成選項參數，和單純參考使用手冊等文件寫死的選項參數相比，或許能夠更符合實際測試使用上的需要。換句話說，動態生成的選項參數或許能和測試時使用的輸入檔產生更好的配合，適時地探索不同的路徑、提高覆蓋率。為了驗證這個推論，我們將蒐集 KOFTA 測試時發現的選項參數、提供給 Wei-fuzz 進行測試，並提出 RQ3：KOFTA 是否能夠幫助 Wei-fuzz 達到更高的測試覆蓋率？
...
\begin{table}[hb]
    ...
    \begin{threeparttable}
        ...
        \begin{tabularx}{0.9\textwidth}{
                ...
                >{\ttfamily\centering\arraybackslash}X}
            \toprule
            \textbf{Program}  & \textbf{\%} & \textbf{\%HasOptarg} & \textbf{\#FuzzOptarg} & \textbf{RQ3} \\
            \midrule
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/1h/objdump-conf-1h.png}
    \end{subfigure}
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/1h/objcopy-conf-1h.png}
    \end{subfigure}
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/1h/readelf-conf-1h.png}
    \end{subfigure}
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/1h/xmllint-conf-1h.png}
    \end{subfigure}
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/1h/tiffcp-conf-1h.png}
    \end{subfigure}
    \begin{subfigure}[]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/1h/tiffcrop-conf-1h.png}
    \end{subfigure}
    \caption{KOFTA 選項參數 v.s. Wei-fuzz 選項參數}
...
最後要探討的是多參數 forkserver 實際帶來的影響。因為 KOFTA 使用污染源推論探索選項參數，因此需要比一般的多參數模糊測試工具更加頻繁地切換目標程式的命令列參數。比起 Wei-fuzz 在切換參數時需要重啟 forserver，KOFTA 實作了可以直接替換命令列參數的 forkserver。為了驗證方法的有效性，我們提出了 RQ4：多參數 forkserver 是否真的能夠減少多參數模糊測試的額外開銷？

在實驗中我們以 KOFTA 與其變體 KOFTA-nf 進行測試比較。變體 KOFTA-nf 在切換命令列參數時會重啟 forkserver（如同 Wei-fuzz）。共有 10 個目標程式，實驗將測量 1 小時內目標程式的「執行次數」。固定時間內的執行次數越高，代表額外開銷越低。從結果（Table \ref{tab:kofta-vs-nf}）中可以看出，使用多參數 forkserver 的 KOFTA 有著更多的執行次數，代表多參數 forkserver 確實降低了污染源推論的額外開銷。與沒有使用多參數 forkserver 的 KOFTA-nf 相比，平均多出了約 44\% 的執行次數。

然而會產生額外開銷的不只有污染源推論，文件變異階段也會。因此這個實驗中我們額外比較了變體 KOFTA-nd 與其變體 KOFTA-nfd 的執行次數。變體 KOFTA-nd 為跳過所有文件變異階段的 KOFTA；而變體 KOFTA-nfd 則是不使用多參數 forkserver 版本的 KOFTA-nd。測試時間同樣為 1 小時，因為沒有文件變異階段，所有的時間將投入在污染源推論中，因此更能突顯污染源推論額外開銷的影響。如同結果（Table \ref{tab:nd-vs-nfd}）所示，使用多參數 forkserver 的版本皆產生了更多的執行次數，其中目標程式 \texttt{giftool} 的效果更是多了高達 347\%。整體而言，與沒有使用多參數 forkserver 的 KOFTA-nfd 相比，平均多出了約 256\% 的執行次數。這個結果與前述使用文件變異階段的 44\% 相比，更是突顯了多參數 forkserver 對於污染源推論的重要性。
...
\begin{table}[hb]
    \centering
    \begin{threeparttable}
        \caption{KOFTA v.s. KOFTA-nf 的執行次數}
...
\begin{table}[hb]
    \centering
    \begin{threeparttable}
        \caption{KOFTA-nd v.s. KOFTA-nfd 的執行次數}
        \label{tab:nd-vs-nfd}
        \begin{tabularx}{0.9\textwidth}{
                >{\ttfamily}c|
                >{\ttfamily\centering\arraybackslash}X
