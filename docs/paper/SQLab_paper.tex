...
% 論文名稱
\newcommand{\chineseTitle}{基於汙染源分析之選項感知模糊測試}
\newcommand{\englishTitle}{KOFTA: Option-Aware Fuzzing with Taint Analysis}

% 自己的名字 (英文名字有兩種寫法, 一個是姓在後, 一個是放前面還有逗號. )
\newcommand{\studentCnName}{陳映達}
\newcommand{\studentEnName}{Ying-TA Chen} % 書名頁
\newcommand{\studentEnNameA}{Chen, Ying-Ta} % 封面用

% 教授的名字
\newcommand{\advisorCnName}{黃世昆}
\newcommand{\advisorEnName}{Shih-Kun Huang}     % 書名頁
\newcommand{\advisorEnNamex}{Huang, Shih-Kun}   % 封面用
...
\newcommand{\ThesisDate}{August 2025}           
\newcommand{\ThesisDateTW}{中華民國~ 一一四年八月} 
...
\newcommand{\code}[2]{{\linespread{1.5}\selectfont\begin{lstlisting}[#1]#2\end{lstlisting}}}
...
\AddToShipoutPicture{
    \put(-5,-15){
        \parbox[b][\paperheight]{\paperwidth}{%
            \vfill
            \centering
            {\includegraphics{covers/Thesis_mark.pdf}}%
            \vfill
        }
    }
}
